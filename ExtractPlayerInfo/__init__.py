...
def get_headers():
    url = 'https://www.capfriendly.com/browse/active/2022/aav?age-calculation-date=today&display=birthday,expiry-year,aav&hide=clauses,age,handed,skater-stats,goalie-stats'
    cap_table_headers = []       
...
def write_cap_data(cap_list, year):
    ...
    cap_data_df.to_csv(upload_file_path, index=False, header=True)
...
def scrape_data(year, last_page):
    ...
    url = 'https://www.capfriendly.com/browse/active/'+ str(cap_year) +'/aav?age-calculation-date=today&display=birthday,expiry-year,aav&hide=clauses,age,handed,skater-stats,goalie-stats&pg='
    ...
    while not new_url.endswith(str(last_page + 1)):  
        ...
        cap_table_contents_slice = [cap_table_contents_text[i:i + 9] for i in range(0, len(cap_table_contents_text), 9)]
